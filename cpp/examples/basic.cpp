...
int main() {
    ...
    auto loaded = ason::load<User>("{id:int,name:str,active:bool}:(1,Alice,true)");
    ...
    // 7. Multiline format
    std::cout << "\n7. Multiline format:\n";
    auto multiline = std::string_view(
    ...
    std::cout << "   original:     User{id=" << original.id << ", name=\"" << original.name << "\"}\n";
    std::cout << "   serialized:   " << serialized << "\n";
    std::cout << "   deserialized: User{id=" << deserialized.id << ", name=\"" << deserialized.name << "\"}\n";
    ...
    std::cout << "   with value: Item{id=" << item1.id
              << ", label=" << (item1.label ? "\"" + *item1.label + "\"" : "nullopt") << "}\n";
    assert(item1.label.has_value());
    assert(*item1.label == "hello");

    auto item2 = ason::load<Item>("{id,label}:(2,)");
    std::cout << "   with null:  Item{id=" << item2.id
              << ", label=" << (item2.label ? "\"" + *item2.label + "\"" : "nullopt") << "}\n";
    ...
    // 11. Comments
    std::cout << "\n11. With comments:\n";
    auto user4 = ason::load<User>("/* user list */ {id,name,active}:(1,Alice,true)");
