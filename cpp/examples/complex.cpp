...
ASON_FIELDS(Street,
    ...
    (buildings, "buildings",  "[{name:str,floors:int,residential:bool,height_m:float}]"))
...
ASON_FIELDS(Region,
    (name,   "name",   "str"),
    (cities, "cities", "[{name:str,population:int,area_km2:float,districts}]"))
...
ASON_FIELDS(SolarSystem,
    (name,      "name",      "str"),
    (star_type, "star_type", "str"),
    (planets,   "planets",   "[{name:str,radius_km:float,has_life:bool,continents}]"))
...
ASON_FIELDS(Universe,
    (name,              "name",              "str"),
    (age_billion_years, "age_billion_years", "float"),
    (galaxies,          "galaxies",          "[{name:str,star_count_billions:float,systems}]"))
...
ASON_FIELDS(ServiceConfig,
    (name,     "name",     "str"),
    (version,  "version",  "str"),
    (db,       "db",       "{host:str,port:int,max_connections:int,ssl:bool,timeout_ms:float}"),
...
template <typename T>
void print_field(const char* label, const T& v) {
    std::cout << "   " << label << ": " << v << "\n";
}
...
struct Measurement { int64_t id = 0; double value = 0; std::string label; };
ASON_FIELDS(Measurement,
    (id,    "id",    "int"),
    (value, "value", "float"),
    (label, "label", "str"))

struct Nums { int64_t a = 0; double b = 0; int64_t c = 0; };
ASON_FIELDS(Nums,
    (a, "a", "int"),
    (b, "b", "float"),
    (c, "c", "int"))

struct WithVec { std::vector<int64_t> items; };
ASON_FIELDS(WithVec, (items, "items", "[int]"))

struct Special { std::string val; };
ASON_FIELDS(Special, (val, "val", "str"))

struct Matrix3D { std::vector<std::vector<std::vector<int64_t>>> data; };
...
int main() {
    ...
    auto emp = ason::load<Employee>("{id,name,dept:{title},skills,active}:(1,Alice,(Manager),[rust],true)");
    std::cout << "   Employee{id=" << emp.id << ", name=" << emp.name
              << ", dept=" << emp.dept.title << ", skills=[";
    for (size_t i = 0; i < emp.skills.size(); i++) { if (i) std::cout << ","; std::cout << emp.skills[i]; }
    ...
    for (auto& [k,v] : wm.attrs) std::cout << k << ":" << v << " ";
    std::cout << "}\n";
    ...
    Country country{
        "Rustland", "RL", 50000000, 1.5,
        {
            Region{"Northern", {
                City{"Ferriton", 2000000, 350.5, {
                    District{"Downtown", 500000, {
                        Street{"Main St", 2.5, {
                            Building{"Tower A", 50, false, 200.0},
                            Building{"Apt Block 1", 12, true, 40.5},
                        }},
                        Street{"Oak Ave", 1.2, {
                            Building{"Library", 3, false, 15.0},
                        }},
                    }},
                    District{"Harbor", 150000, {
                        Street{"Dock Rd", 0.8, {
                            Building{"Warehouse 7", 1, false, 8.0},
                        }},
                    }},
                }},
            }},
            Region{"Southern", {
                City{"Crabville", 800000, 120.0, {
                    District{"Old Town", 200000, {
                        Street{"Heritage Ln", 0.5, {
                            Building{"Museum", 2, false, 12.0},
                            Building{"Town Hall", 4, false, 20.0},
                        }},
                    }},
                }},
            }},
    ...
    auto country2 = ason::load<Country>(s10);
    assert(country2.name == "Rustland");
    assert(country2.regions.size() == 2);
    assert(country2.regions[0].cities[0].districts[0].streets[0].buildings[0].name == "Tower A");
    std::cout << "   ✓ 5-level roundtrip OK\n";

    // 11. 7-level deep
    std::cout << "\n11. Seven-level nesting (Universe>Galaxy>SolarSystem>Planet>Continent>Nation>State):\n";
    Universe universe{
        "Observable", 13.8,
        {Galaxy{"Milky Way", 250.0, {
            SolarSystem{"Sol", "G2V", {
                Planet{"Earth", 6371.0, true, {
                    Continent{"Asia", {
                        Nation{"Japan", {
                            State{"Tokyo", "Shinjuku", 14000000},
                            State{"Osaka", "Osaka City", 8800000},
                        }},
                        Nation{"China", {
                            State{"Beijing", "Beijing", 21500000},
                        }},
                    }},
                    Continent{"Europe", {
                        Nation{"Germany", {
                            State{"Bavaria", "Munich", 13000000},
                            State{"Berlin", "Berlin", 3600000},
                        }},
                    }},
                }},
    ...
    auto universe2 = ason::load<Universe>(s11);
    assert(universe2.name == "Observable");
    assert(universe2.galaxies[0].systems[0].planets[0].continents[0].nations[0].states[0].name == "Tokyo");
    std::cout << "   ✓ 7-level roundtrip OK\n";
    ...
    auto config2 = ason::load<ServiceConfig>(s12);
    assert(config2.name == "my-service");
    assert(config2.db.port == 5432);
    assert(config2.cache.enabled == true);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        for (int r = 0; r < 3; r++) {
            ...
            for (int ci = 0; ci < 2; ci++) {
                City city;
                city.name = "City_" + std::to_string(i) + "_" + std::to_string(r) + "_" + std::to_string(ci);
    ...
    assert(c14.regions[0].cities[0].districts[0].streets[0].buildings[0].name == "HQ");
    ...
    std::cout << "   untyped: " << untyped_len << " bytes | typed: "
              << config_typed.size() << " bytes | overhead: "
              << (config_typed.size() - untyped_len) << " bytes\n";
    ...
    // 18. Comments
    std::cout << "\n18. Comments:\n";
    auto emp18 = ason::load<Employee>(
        "{id,name,dept:{title},skills,active}:/* inline */ (1,Alice,(HR),[rust],true)");
    std::cout << "   with inline comment: Employee{id=" << emp18.id << ", name=" << emp18.name << "}\n";
